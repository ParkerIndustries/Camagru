...
class model_pictures extends model_database {

    public $currentPage;
    public $pagination = '';

    private $artByPage = 20;
    private $numPageLR = 3;
...
//
    ...
    public function addCom($artID, $userID, $com) {
        ...
        // $sendmail = $pdo->prepare("SELECT `Username`,`Email`,`Smail` FROM camagru.t_users INNER JOIN camagru.t_img ON camagru.t_users.ID = camagru.t_img.UserID WHERE camagru.t_img.ArticleID = ?");
           ...
           if ($sql->execute(array(':artID'=>$artID, ':userID'=>$userID, ':com'=>$com)))
            {

                $sendmail = $pdo->prepare("SELECT `ID`, `Username`,`Email`,`Smail` FROM camagru.t_users INNER JOIN camagru.t_img ON camagru.t_users.ID = camagru.t_img.UserID WHERE camagru.t_img.ArticleID = ?");


                if ($sendmail->execute(array($artID)) && $row = $sendmail->fetch())
                {
                    $sendmail = $pdo->prepare("SELECT `Username` FROM camagru.t_users WHERE ID = ?");
                    $sendmail->execute(array($userID));
                    $sender = $sendmail->fetch();
                    // var_dump($sender);

                    if ($row['Smail'])
                    {
                        $subject = "Camagru : ". $sender['Username'] . " commented on your publication !";
                        $header = "From: noreply@camagru.com";
                        $msg = $sender['Username'] .' wrote to your picture : '. $com;
